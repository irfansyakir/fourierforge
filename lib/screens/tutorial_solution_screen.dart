...
class TutorialSolutionScreen extends StatefulWidget {
  const TutorialSolutionScreen({Key? key}) : super(key: key);

  @override
  _TutorialSolutionScreenState createState() => _TutorialSolutionScreenState();
}

class _TutorialSolutionScreenState extends State<TutorialSolutionScreen> {
  // For the input signal visualization
  List<FlSpot> inputSignalPoints = [];
  List<FlSpot> outputSignalPoints = [];
  List<FlSpot> reconstructedSignalPoints = [];
  int numberOfTerms = 5; // Default number of terms
  ...
  void _generateSignalPoints() {
    ...
    for (double x = -1.25; x <= 1.25; x += 0.01) {
      ...
      // This part is simplified as we're removing the reconstruction visualization
      double reconstructedY = 0;
      reconstructedSignalPoints.add(FlSpot(x, reconstructedY));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Tutorial Solution'),
